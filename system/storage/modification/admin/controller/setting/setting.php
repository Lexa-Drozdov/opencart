	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('setting/store', 'user_token=' . $this->session->data['user_token'], true));
				...
				if (isset($this->error['error_config_input_ovt_width'])) {
					$data['error_config_input_ovt_width'] = $this->error['error_config_input_ovt_width'];
				} else {
					$data['error_config_input_ovt_width'] = '';
				}

				if (isset($this->error['error_config_input_ovt_height'])) {
					$data['error_config_input_ovt_height'] = $this->error['error_config_input_ovt_height'];
				} else {
					$data['error_config_input_ovt_height'] = '';
				}
		...
		$data['weight_classes'] = $this->model_localisation_weight_class->getWeightClasses();
				...
				if (isset($this->request->post['config_input_ovt_width'])) {
					$data['config_input_ovt_width'] = html_entity_decode($this->request->post['config_input_ovt_width'], ENT_QUOTES, 'UTF-8');
				} else {
					$data['config_input_ovt_width'] = html_entity_decode($this->config->get('config_input_ovt_width'), ENT_QUOTES, 'UTF-8');
				}
				
				if (isset($this->request->post['config_input_ovt_height'])) {
					$data['config_input_ovt_height'] = $this->request->post['config_input_ovt_height'];
				} else {
					$data['config_input_ovt_height'] = $this->config->get('config_input_ovt_height');
				}
