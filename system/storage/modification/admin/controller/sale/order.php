	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				if (!$this->model_setting_setting->getSettingValue('config_input_ovt_width')) {
					$pi_width = 40;
				} else {
					$pi_width = $this->model_setting_setting->getSettingValue('config_input_ovt_width');
				}
				if (!$this->model_setting_setting->getSettingValue('config_input_ovt_height')) {
					$pi_height = 40;
				} else {
					$pi_height = $this->model_setting_setting->getSettingValue('config_input_ovt_height');
				}

				if (is_file(DIR_IMAGE . $result['image'])) {
					$imagesm = $this->model_tool_image->resize($result['image'], $pi_width, $pi_height);
					$imagelg = $result['image'];
				} else {
					$imagesm = $this->model_tool_image->resize('no_image.png', $pi_width, $pi_height);
					$imagelg = 'no_image.png';
				}
				if ($this->request->server['HTTPS']) {
					$imagelg = HTTPS_CATALOG . 'image/' . $imagelg;
				} else {
					$imagelg = HTTP_CATALOG . 'image/' . $imagelg;
				}
